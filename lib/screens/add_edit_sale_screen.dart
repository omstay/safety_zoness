...
class _AddEditSaleScreenState extends State<AddEditSaleScreen> {
  ...
  void initState() {
    ...
    if (widget.saleDoc != null) {
      final data = widget.saleDoc!.data() as Map<String, dynamic>;
  ...
  Future<void> _saveSale({bool saveAndNew = false}) async {
    ...
    if (widget.saleDoc == null) {
      saleRef = await FirebaseFirestore.instance.collection('sales').add(saleData);
    } else {
      saleRef = widget.saleDoc!.reference;
      await saleRef.update(saleData);
      final oldItems = await saleRef.collection('items').get();
  ...
  void _shareInvoice() {
    final invoiceText = StringBuffer()
      ...
      ..writeln("Items:");
    for (var item in _items) {
      invoiceText.writeln("- ${item['name']} (${item['quantity']} ${item['unit']}) x ₹${item['price']}");
    }
                ...
                children: [
                  Expanded(
                    child: DropdownButtonFormField<int>(
                      value: invoiceNumber,
                      items: [invoiceNumber]
                          .map((e) => DropdownMenuItem(value: e, child: Text('Invoice No. $e')))
                          .toList(),
                      onChanged: null,
